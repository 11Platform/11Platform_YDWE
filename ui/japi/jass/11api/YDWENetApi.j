...
//显示玩家的排行，如果不存在则返回-1
function YDWEGetPalyerRPGRank takes integer player_id returns integer
     return GetStoredInteger(ranktable, "PlayerRank", I2S(player_id) )
endfunction

//显示玩家的分数，如果不存在则返回-1
function YDWEGetPalyerRPGRankScore takes integer player_id returns integer
    return GetStoredInteger(ranktable, "PlayerScore", I2S(player_id) )
endfunction
...
function YDWESaveRemoteData takes integer player_id, string Key ,string value returns boolean
    if is_11Platform == true then
        return EXNetSaveRemoteData(player_id, Key, value)
    else
        return EXSaveRemoteData(player_id, Key, value)
    endif
endfunction

function PlayerHighFreqScoreTest takes integer player_id, string value returns nothing
    call YDWESaveRemoteData(player_id,"HFreqT1",value)
    call YDWESaveRemoteData(player_id,"HFreqT2",value)
    call YDWESaveRemoteData(player_id,"HFreqT3",value)
    call YDWESaveRemoteData(player_id,"HFreqT4",value)
    call YDWESaveRemoteData(player_id,"HFreqT5",value)
    call YDWESaveRemoteData(player_id,"HFreqT6",value)
    call YDWESaveRemoteData(player_id,"HFreqT7",value)
    call YDWESaveRemoteData(player_id,"HFreqT8",value)
    call YDWESaveRemoteData(player_id,"HFreqT9",value)
    call YDWESaveRemoteData(player_id,"HFreqT10",value)
endfunction
